...
namespace SolidTacToe.Tests.GameOverConditions
{
    ...
    public class VerticalWinConditionTest
    {
        ...
        public class ConditionMet
        {
            ...
            private static Mock<IGrid> GetEmptyGridMock()
            {
                const int size = 3;
            ...
            [TestMethod]
            public void TrueForVerticalWin()
            {
                var grid = GetEmptyGridMock();
                ...
                var condition = new VerticalWinCondition { Grid = grid.Object, Winner = Token.X};
                var result = condition.ConditionMet();

                Assert.IsTrue(result);
            }

            [TestMethod]
            public void FalseForHorizontalWin()
            {
                var grid = new Mock<IGrid>();
